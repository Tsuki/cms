  ...
  var methods = {
    ...
    init: function (options) {
      this.each(function () {
        ...
        $viewport = $obj.find(".als-viewport").attr("data-id",
            "als-viewport_" + instance);
        $wrapper = $obj.find(".als-wrapper").attr("data-id",
            "als-wrapper_" + instance);
        ...
        switch ($options.orientation) {
          ...
          case "vertical":
            ...
            if ($options.circular == "yes" && $options.start_from != 0) {
              ...
              for (r = 0; r < $options.start_from; r++) {
                var position = $item.last().position(),
                    bottom_repos = position.top + $item.last().outerHeight(
                            true);
        ...
        $viewport.on('touchend', function (e) {
          ...
          if ($options.orientation == "horizontal") {
            if (mm.dx < -mm.swipeTreshold && mm.endTime < mm.allowedTime) {
              ...
              if ($options.circular == "yes" || $options.visible_items
                  + $options.start_from < num_items) {
        ...
        default:
          ...
          switch (data.options.circular) {
            ...
            default:
               ...
               ********************************************************************/
              for (k1 = data.current;
                  k1 < data.current + data.options.scrolling_items; k1++) {
               ...
               *******************************************************************/
              for (k2 = data.current;
                  k2 < data.current + data.options.visible_items; k2++) {
              ...
              data.item.promise().done(function () {
                ...
                data.viewport.on('touchend', function (e) {
                  ...
                  if (data.options.orientation == "horizontal") {
                    if (mm.dx < -mm.swipeTreshold && mm.endTime
                        < mm.allowedTime) {
                      // scroll on swipe only of there are enough elements
                      if (data.current + data.options.visible_items
                          < data.num_items) {
                        nextHandle(e, data.viewport);
                      }
                    }
                    else if (mm.dx > mm.swipeTreshold && mm.endTime
                        < mm.allowedTime) {
                    ...
                    if (mm.dy < -mm.swipeTreshold && mm.endTime
                        < mm.allowedTime) {
                      // scroll on swipe only of there are enough elements
                      if (data.current + data.options.visible_items
                          < data.num_items) {
                    ...
                    else if (mm.dy > mm.swipeTreshold && mm.endTime
                        < mm.allowedTime) {
                      // scroll back on swipe only if there are enough elements
                      if (data.current > 0) {
                        prevHandle(e, data.viewport);
            ...
            case "yes":
              ...
              data.item.promise().done(function () {
                ...
                var position = data.item.last().position(),
                    right_repos = position.left + data.item.last().outerWidth(
                            true);
                for (k5 = 0; k5 < memo_index.length; k5++) {
                  if (memo_index[k5] == 0) {
                    var position = data.item.last().position(),
                        right_repos = position.left
                            + data.item.last().outerWidth(true);
                  }
          ...
          switch (data.options.circular) {
            ...
            case "yes":
              ...
              data.item.promise().done(function () {
                 ...
                 ************************************************************************/
                var position = data.item.last().position(),
                    bottom_repos = position.top + data.item.last().outerHeight(
                            true);
                for (k5 = 0; k5 < memo_index.length; k5++) {
                  if (memo_index[k5] == 0) {
                    var position = data.item.last().position(),
                        bottom_repos = position.top
                            + data.item.last().outerHeight(true);
                  }
